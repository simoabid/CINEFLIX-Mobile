...
import { View, Text, ScrollView, Dimensions, Pressable, Image, Linking, Share, Modal } from "react-native";
// import { Image } from "expo-image"; // Disabling expo-image temporarily to fix rendering issues
import { LinearGradient } from "expo-linear-gradient";
import { useEffect, useState, useRef } from "react";
import * as tmdb from "../../services/tmdb";
import YoutubePlayer from "react-native-youtube-iframe";
import { Movie, MovieCredits, Video, CastMember, CrewMember, TVShow, Season, Episode, WatchProvidersResponse, ExternalIds } from "../../types";
import { ChevronLeft, Play, Plus, Share2, Star, Clock, Calendar, X, Info, Facebook, Instagram, Twitter, ExternalLink, ChevronDown, Check, PlayCircle, Globe, DollarSign, Film, Languages, Wallet, Trophy, User, Maximize, Minimize } from "lucide-react-native";
...
export default function DetailScreen() {
    ...
    useEffect(() => {
        if (mediaType === 'tv' && selectedSeasonNumber !== null) {
        ...
        try {
            if (mediaType === 'movie') {
                const [details, creds, vids, sims, providers, extIds] = await Promise.all([
                    tmdb.getMovieDetails(contentId),
                    tmdb.getMovieCredits(contentId),
                    tmdb.getMovieVideos(contentId),
                    tmdb.getSimilarMovies(contentId),
                    tmdb.getWatchProviders(contentId, 'movie'),
                    tmdb.getMovieExternalIds(contentId)
                ]);
                setContent(details);
                setCredits(creds);
                setVideos(vids.filter(v => v.site === "YouTube"));
                setSimilar(sims.results);
                ...
                if (details.seasons && details.seasons.length > 0) {
                    ...
                    // Select first season usually, or season 1 if strictly numbered
    ...
    const loadEpisodes = async (seasonNum: number) => {
        ...
        try {
            // tmdb.getTVShowSeasonDetails returns the full object with episodes array
    ...
    const handleShare = async () => {
        try {
            const title = mediaType === 'movie' ? (content as Movie).title : (content as TVShow).name;
            await Share.share({
                message: `Check out ${title} on CineFlix!`,
                url: `https://www.themoviedb.org/${mediaType}/${contentId}`
            });
        } catch (error) {
            // ignore
        }
    ...
    const year = mediaType === 'movie'
        ? ((content as Movie).release_date ? new Date((content as Movie).release_date).getFullYear() : '')
        : ((content as TVShow).first_air_date ? new Date((content as TVShow).first_air_date).getFullYear() : '');

    // Logic for runtime/seasons text
    let metaText = '';
    if (mediaType === 'movie') {
        const r = (content as Movie).runtime;
        metaText = r ? `${Math.floor(r / 60)}h ${r % 60}m` : '';
    } else {
        const s = (content as TVShow).number_of_seasons;
        metaText = `${s} Season${s !== 1 ? 's' : ''}`;
    }

    const displayedCast = activeTab === 'cast' ? credits?.cast : credits?.crew.filter(c => ['Director', 'Producer', 'Writer'].includes(c.job));
        ...
        <View className="flex-1 bg-netflix-black">
            ...
            <ScrollView className="flex-1" showsVerticalScrollIndicator={false} bounces={false}>
                ...
                <View className="px-4 -mt-2 pb-12">
                        ...
                        <View className="mb-8">
                            ...
                            {loadingEpisodes ? (
                                <View className="py-8 items-center">
                                    <Text className="text-gray-400">Loading episodes...</Text>
                                </View>
                            ) : (
                        ...
                        <View className="mb-20">
                            ...
                            <View className="flex-row flex-wrap justify-between">
                                {similar.slice(0, 9).map((item) => {
                                    const date = 'first_air_date' in item ? item.first_air_date : item.release_date;
                                    const year = date ? new Date(date).getFullYear() : '';
                                    return (
                                        <Pressable
                                            key={item.id}
                                            className="w-[32%] mb-4"
                                            onPress={() => router.push({ pathname: "/movie/[id]", params: { id: item.id, type: mediaType } })} // Keep same type for recommendations usually
                                        >
                                            ...
                                            <View className="flex-row items-center justify-between px-1">
                                                <Text className="text-gray-400 text-[10px] font-medium">{year}</Text>
                                                {item.vote_average > 0 && (
                        ...
                        <View className="mb-8 p-4 bg-gray-900/50 rounded-2xl border border-white/5 mx-[-4]">
                            ...
                            <View className="gap-3">
                                ...
                                {(!watchProviders.results.US.flatrate || watchProviders.results.US.flatrate.length === 0) && (
                                    <Text className="text-gray-500 text-sm">Not currently streaming in US.</Text>
                                )}
                            </View>
                        </View>
                    )}
                    ...
                    <View className="mb-8">
                        ...
                        <View className="bg-gray-900/50 rounded-2xl p-4 border border-white/5 space-y-4">
                            ...
                            <View className="flex-row items-center gap-3 pb-3 border-b border-white/5">
                                ...
                                <View>
                                    <Text className="text-gray-400 text-xs uppercase font-bold tracking-wider">Runtime</Text>
                                    <Text className="text-white font-bold text-lg">{metaText}</Text>
                                </View>
                            </View>
                            ...
                            {credits?.crew.find(c => c.job === 'Director') && (
                                <View className="flex-row items-center gap-3">
                                    ...
                                    <View>
                                        <Text className="text-gray-400 text-xs uppercase font-bold tracking-wider">Director</Text>
                                        <Text className="text-white font-bold text-lg">{credits.crew.find(c => c.job === 'Director')?.name}</Text>
                                    </View>
                                </View>
                            )}
                        </View>
                    ...
                    {/* Detailed Info Cards (Budget, Revenue, Status) */}
                    <View className="mb-8">
                        <View className="flex-row items-center gap-2 mb-4">
                            <View className="w-1 h-4 bg-netflix-red rounded-full" />
                            ...
                            <View className="bg-gray-900/50 p-4 rounded-xl border border-white/5 flex-row items-center justify-between">
                                <View className="flex-row items-center gap-3">
                                    ...
                                    <View>
                                        ...
                                        <Text className="text-white font-semibold">{content.status || 'Released'}</Text>
                                    </View>
                                </View>
                            </View>
                            ...
                            <View className="bg-gray-900/50 p-4 rounded-xl border border-white/5 flex-row items-center justify-between">
                                <View className="flex-row items-center gap-3">
                                    ...
                                    <View>
                                        ...
                                        {/* Spoken languages is array, show first or original language */}
                    ...
                    <View className="mb-12">
                        ...
                        <View className="gap-3">
                            {externalIds?.instagram_id && (
                                ...
                                >
                                    ...
                                    <View>
                                        ...
                                        <Text className="text-gray-500 text-xs">Follow regarding photos</Text>
                                    </View>
                            ...
                            {(content as Movie).homepage && (
                                <Pressable
                                    onPress={() => Linking.openURL((content as Movie).homepage!)}
                                    className="flex-row items-center gap-4 bg-gray-900/50 p-4 rounded-xl border border-white/5"
                                >
