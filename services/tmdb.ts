...
import { Movie, TVShow, Video, ApiResponse, Content, Genre, ImageConfig, Collection, CollectionDetails, CollectionType, CollectionStatus, MovieCredits, PersonDetails, PersonMovieCredits, ExternalIds, WatchProvidersResponse } from '../types';
...
tmdbApi.get = async (url: string, config: AnyObject = {}): Promise<any> => {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    try {
      ...
      console.error("[TMDB Service Error] URL:", fullUrl);
      console.error("[TMDB Service Error] Message:", err.message);
      if (err.response) {
        console.error("[TMDB Service Error] Status:", err.response.status);
        console.error("[TMDB Service Error] Data:", JSON.stringify(err.response.data));
      } else {
        console.error("[TMDB Service Error] No response received. Possible network/CORS/DNS issue.");
      }
...
export const getWatchProviders = async (id: number, type: 'movie' | 'tv' = 'movie'): Promise<WatchProvidersResponse> => {
...
export const searchMulti = async (query: string, page: number = 1): Promise<ApiResponse<Content>> => {
